...
+ (void)load
{
    ...
    if (![args containsObject:@"-hud"])
    {
        return;
    }

    // HUD version now controls ImGui menu visibility from a dedicated HUD
    // controller. Disable old in-game gesture toggles to avoid unexpected
    // show/hide when tapping on the screen.
    return;
